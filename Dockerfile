...
RUN bash -c 'source /opt/ros/kinetic/setup.bash && export ROS_PACKAGE_PATH=${ROS_PACKAGE_PATH}:/opt/ORB_SLAM2/Examples/ROS/ && cd /opt/ORB_SLAM2/Examples/ROS && rosmake ORB_SLAM2'

RUN mkdir -p /catkin_ws/src && cd /catkin_ws/src/ && git clone https://github.com/ros-drivers/video_stream_opencv.git
RUN cd /catkin_ws/src/video_stream_opencv && rosdep update && rosdep install --from-path /catkin_ws/src/video_stream_opencv --ignore-src
